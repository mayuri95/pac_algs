...
C_range = [x / 10 for x in range(1, 11)]
    ...
    for C in C_range:
        ...
        for i in range(num_trials):
            ...
            for ind in range(len(svm_vec)):
                c = np.random.normal(0, scale=cifar10_noise[C][ind])
                svm_vec[ind] += c
        ...
        cifar10_acc[C] = (avg_orig_acc, avg_priv_acc)
        print(f'acc={avg_orig_acc}, {avg_priv_acc}')

    with open(f'test_data/cifar10_svm_acc_auto_s=0.5_mi={mi}.pkl', 'wb') as f:
        pickle.dump(cifar10_acc, f)
        
    with open(f'test_data/cifar10_svm_noise_auto_s=0.5_mi={mi}.pkl', 'wb') as f:
