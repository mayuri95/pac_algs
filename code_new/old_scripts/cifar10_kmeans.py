...
train_x, train_y, test_x, test_y, num_classes, train_len = gen_cifar10()

subsample_rate = int(0.1*train_len)
...
for mi in mi_range:
    ...
    cifar10_noise[mi] = est_noise

    with open(f'test_data/kmeans_cifar10_noise_mi={mi}.pkl', 'wb') as f:
        pickle.dump(cifar10_noise, f)
    print('cifar10 noise complete')


    cifar10_num_features = len(train_x[0])
    num_trials = 1000
    ...
    for i in range(num_trials):
        ...
        for ind in range(len(cluster_centers)):
            c = np.random.normal(0, scale=cifar10_noise[mi][ind])
            cluster_centers[ind] += c
        new_centers = np.reshape(cluster_centers, (num_classes, cifar10_num_features))
    ...
    cifar10_acc[mi] = (avg_orig_acc, orig_acc_var, avg_priv_acc, priv_acc_var)

with open(f'test_data/kmeans_cifar10_auto_s=0.5_acc.pkl', 'wb') as f:
    pickle.dump(cifar10_acc, f)
print(mi, cifar10_acc)
