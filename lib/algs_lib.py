...
def transform(s1, s2):
    ...
    u, sigma, v = np.linalg.svd(np.matmul(s2, s1.T))

    end_shape = s2.shape[0]
    c_mat = np.matmul(v.T, u.T)
...
def hybrid_noise_auto(train_x, train_y, mechanism, subsample_rate, num_classes,
    eta, regularize=None, rebalance = False, num_trees=None, tree_depth = None, max_mi = 0.5, num_dims = None,
    record_ys = False, fname = None, max_num_trials = None):
                ...
                if prev_ests is None:
                    ...
                    if not converged:
                        for ind in est_y:
                            prev_ests[ind] = np.var(est_y[ind])
        curr_trial += 1
        if max_num_trials is not None and curr_trial > max_num_trials:
            break
    fin_var = {ind: np.var(est_y[ind]) for ind in est_y}
...
def hybrid_noise_mean(train_x, train_y, subsample_rate, num_classes, eta, max_mi = 0.5):
...
def hybrid_noise_mean_ind(train_x, train_y, subsample_rate, num_classes, eta, max_mi = 0.5):
    ...
    for chosen_ind in range(len(train_x)):
        # print(f'ind is {chosen_ind}')
        converged = False
        curr_trial = 0
        est_y = {}
        prev_ests = None
        removed_train_x = np.delete(train_x, chosen_ind, 0)
        removed_train_y = np.delete(train_y, chosen_ind, 0)
        while not converged:
            shuffled_x, shuffled_y = shuffle(removed_train_x, removed_train_y)

            shuffled_x, shuffled_y = get_samples_safe(shuffled_x, shuffled_y, num_classes, subsample_rate)

            ind_to_replace = np.random.choice(range(len(shuffled_x)))
            added_x = list(copy.deepcopy(shuffled_x))

            added_x[ind_to_replace] = train_x[chosen_ind]
            ...
            if curr_trial % 10 == 0 and curr_trial > 100:
                if prev_ests is None:
                    prev_ests = {}
                    for ind in est_y:
...
class DecisionTree(object):
    ...
    def add_noise_aniso(self, noise, train_x, train_y, node=None, offset=0):
        ...
        if node == self.root:
            ...
            for ind in range(len(all_nodes)):
                ...
                all_nodes[ind].split_condition = (orig_feat, orig_value + np.random.normal(0, scale=noise[ind+offset]))
...
class Forest(object):
    ...
    def add_noise_aniso(self, noise, x, y):
        ...
        for i in range(len(self.trees)):
            all_nodes = self.trees[i].add_noise_aniso(noise, x, y, offset=offset)
...
def run_pca(train_x, train_y, num_dims, seed):
    rand_state = np.random.RandomState(seed)
    model = PCA(n_components=num_dims, random_state=rand_state)
    model.fit(train_x)
    return model, model.components_

# GENERATE DATA (SYNTHETIC)

norms = ['standard', 'standard_no_std', 'standard_no_std_no_mean', 'power', 'power_standard',
         'minmax', 'quantile', 'quantile_gaussian', 'robust', 'robust_no_scale', 'robust_no_scale_no_center']

from sklearn import preprocessing
def gen_iris(normalize=False, norm_kind = None):
    iris = fetch_ucirepo(id=53)
    # data (as pandas dataframes)
    if normalize:
        if norm_kind == 'standard':
            pt = preprocessing.StandardScaler()
        elif norm_kind == 'standard_no_std':
            pt = preprocessing.StandardScaler(with_std=False)
        elif norm_kind == 'standard_no_std_no_mean':
            pt = preprocessing.StandardScaler(with_std=False, with_mean=False)
        elif norm_kind == 'power':
            pt = preprocessing.PowerTransformer(standardize=False)
        elif norm_kind == 'power_standard':
            pt = preprocessing.PowerTransformer(standardize=True)
        elif norm_kind == 'minmax':
            pt = preprocessing.MinMaxScaler()
        elif norm_kind == 'quantile':
            pt = preprocessing.QuantileTransformer(random_state=0)
        elif norm_kind == 'quantile_gaussian':
            pt = preprocessing.QuantileTransformer(output_distribution='normal', random_state=0)
        elif norm_kind == 'robust':
            pt = preprocessing.RobustScaler()
        elif norm_kind == 'robust_no_scale':
            pt = preprocessing.RobustScaler(with_scaling=False)
        elif norm_kind == 'robust_no_scale_no_center':
            pt = preprocessing.RobustScaler(with_scaling=False, with_centering=False)
        elif norm_kind == 'normalizer':
            pt = preprocessing.Normalizer(norm='l1')
        else:
            print(norm_kind)
            assert(False)
        scaled = pt.fit_transform(iris.data.features)
...
def gen_rice(normalize=False, use_file = False, norm_kind=None):
    ...
    if normalize:
        ...
        scaled = pt.fit_transform(rice.data.features)
        X = pd.DataFrame(scaled)
    else:
...
def gen_cifar10(normalize=False, norm_kind=None):
...
def gen_bean(normalize=False, norm_kind=None):
    # fetch dataset
    ...
    if normalize:
        if norm_kind == 'standard':
            ...
            print(norm_kind)
            assert(False)
        scaled = pt.fit_transform(dry_bean.data.features)
        X = pd.DataFrame(scaled)
    else:
        X = dry_bean.data.features
    all_x = X.to_numpy()
