...
for mi in mis:
    ...
    for C_i, dim in enumerate(dims):
        with open(f'noise/bean_pca_noise_auto_dim={dim}.pkl', 'rb') as f:
    ...
    with open(f'accs/bean_pca_dist_mi_{mi}.pkl', 'wb') as f:
...
for mi in mis:
    ...
    for C_i, dim in enumerate(dims):
        ...
        acc_dict[dim] = (avg_orig_acc, avg_priv_acc)
        print(f'dim={dim}, mi={mi}, norm={norm}, acc={avg_orig_acc}, {avg_priv_acc}')
    with open(f'accs/bean_iso_pca_dist_mi_{mi}.pkl', 'wb') as f:
        pickle.dump(acc_dict, f)
