...
with open(f'noise/rice_svm_noise.pkl', 'rb') as f:
    all_noise = pickle.load(f) # keyed by MI, default is 0.5

for mi in mi_range:
    acc_dict = {}
    for C_i, C in enumerate(C_range):
        train_x, train_y, test_x, test_y, num_classes, train_len = gen_rice(normalize=True, norm_kind =norms[C_i])

        subsample_rate = int(0.5*train_len)
        orig_noise, seed = all_noise[(C, norms[C_i])]
        ...
        print(f'C={C}, mi={mi}, acc={avg_orig_acc}, {avg_priv_acc}')

    with open(f'accs/rice_acc_mi={mi}.pkl', 'wb') as f:
...
for mi in mi_range:
    ...
    with open(f'accs/rice_iso_acc_mi={mi}.pkl', 'wb') as f:
        pickle.dump(acc_dict, f)
