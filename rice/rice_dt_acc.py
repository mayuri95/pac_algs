...
mi_range = mi_range[start_ind:start_ind+2]
...
for reg_i, reg in enumerate(regs):
    ...
    with open(f'noise/rice_dt_noise_reg={reg}_norm={norm}.pkl', 'rb') as f:
    ...
    for mi in mi_range:
        ...
        print(f'rice acc (reg={reg}, norm={norm}, aniso, mi={mi}): {(avg_orig_acc, avg_priv_acc)}')

        with open(f'accs/rice_acc_auto_reg={reg}_mi={mi}_norm={norm}.pkl', 'wb') as f:
...
for reg_i, reg in enumerate(regs):
    ...
    for mi in mi_range:
        ...
        with open(f'accs/rice_iso_acc_auto_reg={reg}_mi={mi}_norm={norm}.pkl', 'wb') as f:
            pickle.dump(acc_dict, f)
