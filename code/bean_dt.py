...
for reg in regularizations:
    for mi in mi_range:
        ...
        noise = {}
        est_noise = hybrid_noise_auto(train_x, train_y, fit_forest, subsample_rate, eta=1e-2,
            num_classes = num_classes, max_mi=mi, regularize=reg, num_trees = num_trees, tree_depth=tree_depth)
        noise[reg] = est_noise
        print(f'bean noise {est_noise}')

        with open(f'test_data/bean_noise_auto_reg={reg}_mi={mi}.pkl', 'wb') as f:
            pickle.dump(noise, f)    
        ...
        with open(f'test_data/bean_acc_auto_s=0.5_reg={reg}_mi={mi}.pkl', 'wb') as f:
