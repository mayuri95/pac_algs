...
for mi in mi_range:
    ...
    with open(f'test_data/kmeans_bean_noise_mi={mi}.pkl', 'wb') as f:
        pickle.dump(noise, f)
    ...
    for i in range(num_trials):
        ...
        for ind in range(len(cluster_centers)):
            c = np.random.normal(0, scale=noise[mi][ind])
    ...
    accs[mi] = (avg_orig_acc, orig_acc_var, avg_priv_acc, priv_acc_var)

with open(f'test_data/kmeans_bean_auto_s=0.5_mi={mi}_acc.pkl', 'wb') as f:
    pickle.dump(accs, f)
