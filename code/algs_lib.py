...
def hybrid_noise_mean_ind(train_x, train_y, subsample_rate, num_classes,
    ...
    while curr_trial < num_trials:
        ...
        all_xs.append(np.average(x1, axis=0))
        all_xs.append(np.average(x2, axis=0))
        curr_trial += 2

    
    all_xs = np.array(all_xs)

    pairwise_distances = np.zeros((len(all_xs), len(all_xs)))

    for row_ind in range(len(all_xs)):
        for col_ind in range(len(all_xs)):
            pairwise_distances[row_ind][col_ind] = np.linalg.norm(all_xs[row_ind] - all_xs[col_ind])
    ...
    for ind in range(len(train_x)):
        ...
        for set_ind in xs_in:
            ...
            for k in xs_out:
                dist = pairwise_distances[set_ind][k]
