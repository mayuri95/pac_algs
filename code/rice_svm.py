...
for mi in mi_range:
    ...
    for C in C_range:
        ...
        for i in range(num_trials):
            ...
            for ind in range(len(svm_vec)):
                c = np.random.normal(0, scale=rice_noise[C][ind])
                svm_vec[ind] += c
    ...
    with open(f'test_data/rice_svm_acc_auto_s=0.5_mi={mi}.pkl', 'wb') as f:
        pickle.dump(rice_acc, f)
        
    with open(f'test_data/rice_svm_noise_auto_s=0.5_mi={mi}.pkl', 'wb') as f:
