...
for mi in mi_range:
    ...
    for C in C_range:
        ...
        est_noise = rand_mechanism_noise(train_x, train_y, run_svm, subsample_rate, tau=3,
                                         num_classes = num_classes, regularize=C, max_mi=mi)[2]
        rice_noise[C] = est_noise
        ...
        for i in range(num_trials):
            ...
            for ind in range(len(svm_vec)):
                c = np.random.normal(0, scale=rice_noise[C])
                svm_vec[ind] += c
        ...
        rice_acc[C] = (avg_orig_acc, avg_priv_acc)
        print(f'acc={avg_orig_acc}, {avg_priv_acc}')

    with open(f'data_0120/rice_svm_acc_mi={mi}.pkl', 'wb') as f:
        pickle.dump(rice_acc, f)
        
    with open(f'data_0120/rice_svm_noise_mi={mi}.pkl', 'wb') as f:
