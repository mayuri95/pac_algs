...
for reg in regs:
    with open(f'hybrid_dt/bean_noise_auto_reg={reg}.pkl', 'rb') as f:
        orig_noise, seed = pickle.load(f)[reg]
    for mi in mi_range:
        acc_dict = {}
        ...
        print(f'bean acc (reg={reg}, aniso, mi={mi}): {(avg_orig_acc, avg_priv_acc)}')

        with open(f'hybrid_dt/bean_acc_auto_reg={reg}_mi={mi}.pkl', 'wb') as f:
...
for reg in regs:
    ...
    for mi in mi_range:
        ...
        avg_priv_acc /= num_trials
        acc_dict[reg] = (avg_orig_acc, avg_priv_acc)
        print(f'bean acc (reg={reg}, iso, mi={mi})= {(avg_orig_acc, avg_priv_acc)}')

        with open(f'hybrid_dt/bean_iso_acc_auto_reg={reg}_mi={mi}.pkl', 'wb') as f:
            pickle.dump(acc_dict, f)
