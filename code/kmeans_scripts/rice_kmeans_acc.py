...
for rebalance in [True, False]:
    with open(f'hybrid_kmeans/rice_kmeans_hybrid_auto_s=0.5_noise_rebalance={rebalance}.pkl', 'rb') as f:
    ...
    with open(f'hybrid_kmeans/rice_kmeans_acc_bal={rebalance}.pkl', 'wb') as f:
...
for rebalance in [True, False]:
    ...
    for mi in mi_range:
        ...
        for i in range(num_trials):
            ...
            model, cluster_centers = run_kmeans(shuffled_x1, shuffled_y1, num_clusters=num_classes, seed=seed, rebalance=rebalanced)
            predictions = model.predict(test_x)
            acc = accuracy_score(test_y, predictions)
            avg_orig_acc += acc
            for ind in range(len(cluster_centers)):
        ...
        acc_dict[mi] = (avg_orig_acc, orig_acc_var, avg_priv_acc, priv_acc_var)
    with open(f'hybrid_kmeans/rice_iso_kmeans_acc_bal={rebalance}.pkl', 'wb') as f:
        pickle.dump(acc_dict, f)